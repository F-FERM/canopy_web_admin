...
import { listSeviceSecurityServicesApi } from "@/app/api/HomeService";
...
const ServiceDetailPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchDetail = async () => {
      try {
        ...
        const matchingService = root.services.find((s: any) => s.slug === id || s._id === id);
  ...
  const { heroSection, whyChooseSection, responsibilitiesSection, industriesSection } = data;

  // Map API shape → component props
  const responsibilityItems = responsibilitiesSection?.responsibilities?.map((item: any) => ({
    title: item.title,
    desc: item.description,
  })) || [];

  const industryItems = industriesSection?.industries?.map((item: any) => ({
    title: item.title,
    desc: item.description,
  })) || [];
    ...
    <div>
      ...
      {whyChooseSection && (
        <WhyCCTVOperators
          ...
          paragraphs={[whyChooseSection.descriptionOne, whyChooseSection.descriptionTwo].filter(Boolean)}
