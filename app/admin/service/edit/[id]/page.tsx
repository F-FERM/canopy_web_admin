...
export default function EditServiceLandingPage() {
  ...
  const [initialData, setInitialData] =
    useState<Partial<ServiceLandingSection> | null>(null);
    ...
    const load = async () => {
      try {
        ...
        const found = all.find(
          (item: ServiceLandingSection) => item._id === id,
        );
  ...
  if (!initialData)
    return (
      <div className="p-8 text-center text-gray-500">
        Item not found.{" "}
        <Link href="/admin/service" className="text-blue-600 underline">
          Go back
        </Link>
      </div>
    );
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-800">
            Edit Service Landing
          </h1>

          <p className="text-gray-500">Update service landing section.</p>
        </div>
      </div>
