...
import { createProcessSection } from "@/app/api/admin/service/serviceProcess";
...
export default function CreateProcessSectionPage() {
  ...
  const handleSubmit = async (data: any) => {
    try {
      await createProcessSection(data);

      router.push("/admin/service/service-process");
    } catch (err) {
      console.error("Create error:", err);

      alert("Failed to create process section");
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-500">Create process section.</p>
        </div>
      </div>

      <div className="bg-gray-50 p-6 rounded-xl border border-dashed">
        <ProcessSectionForm onSubmit={handleSubmit} />
      </div>
    </div>
  );
}
