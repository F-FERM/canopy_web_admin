...
export default function ProcessSectionPage() {
  const [data, setData] = useState<ProcessSection[]>([]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Process Section</h1>

          <p className="text-gray-500">Manage process section.</p>
        </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-gray-50 border-b">
              ...
              <th className="p-4 font-semibold text-gray-700">Processes</th>
          ...
          <tbody>
            {data.length > 0 ? (
              data.map((item) => {
                const activeCount =
                  item.processes?.filter((p) => p.isActive).length || 0;
                ...
                return (
                  ...
                  >
                    ...
                    <td className="p-4 text-sm text-gray-700">
                      {activeCount} active / {item.processes?.length} total
                    </td>
